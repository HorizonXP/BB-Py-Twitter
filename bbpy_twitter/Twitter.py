...
class Twitter(OAuthProvider):
    ...
    def getUserProfileData(self):
        if self.authorized:
            ...
            self._curUser = Twitter.User(self._twitterApi.VerifyCredentials())
            self.userChanged.emit()
            statuses = self._twitterApi.GetPublicTimeline()
            self._publicTimeline = Twitter.TimelineModel([Twitter.Status(s) for s in statuses])
            self.publicTimelineUpdated.emit()
            statuses = self._twitterApi.GetUserTimeline()
            self._userTimeline = Twitter.TimelineModel([Twitter.Status(s) for s in statuses])
            self.userTimelineUpdated.emit()
            statuses = self._twitterApi.GetFriendsTimeline()
            self._friendsTimeline = Twitter.TimelineModel([Twitter.Status(s) for s in statuses])
    ...
    class User(QObject):
        def __init__(self, user):
            super(Twitter.User, self).__init__()
        ...
        def _getStatus(self):
            return Twitter.Status(self.user.status)
        status = Property(Status, _getStatus, notify=statusChanged)
    ...
    class Status(QObject):
        def __init__(self, status):
            super(Twitter.Status, self).__init__()
        ...
        def _getUser(self):
            return Twitter.User(self.status.user)
